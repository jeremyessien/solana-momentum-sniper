...
In production, logs are written to standard output as structured JSON. The deployment runtime captures, rotates, and persists them — on Hetzner, this is systemd capturing stdout into journald, with retention configured at the journald level. This matches the modern systemd-native pattern and removes a class of in-process failure modes that come with managing rotation ourselves. If we ever deploy to an environment without a runtime that captures stdout, we revisit this and add file-based rotation in-process via a library like pino-roll. Any local log files that result, in development or otherwise, are not committed to version control and are excluded from any deployment artifact.
