...
Phase 1 ships with one strategy: `data-collection`. Its thresholds are permissive — chosen to produce a useful distribution of `Pass` / `Watch` / `Enter` labels across observed tokens, not chosen to make money. No execution layer subscribes to its decisions in Phase 1; every decision is recorded into the data store and the analytical work begins from there. The shape of the decision record, the choice to hoist thresholds onto the wrapper rather than embed them in reason variants, and the two-stage exclusions-then-signal filter the strategy applies are all captured in ADR-007.
...
A `StrategyEvaluationResult` is a wrapper carrying the `strategyId` that produced the verdict, the `TokenWithFullContext` candidate that was evaluated, a snapshot of the threshold parameters that were applied, the decision itself, and an `evaluatedAt` timestamp. The decision is a discriminated union with three variants. A `Pass` variant carries a non-empty array of reason codes describing every rule that failed — multi-reason capture is deliberate because most rejected tokens fail multiple checks at once. A `Watch` variant carries a non-empty array of borderline reasons. An `Enter` variant carries no extra fields in Phase 1; Phase 2's execution layer will extend it with position size and exit rules, and the discriminated union pattern ensures the type system rejects an "enter" decision lacking those fields when the time comes. ADR-007 captures the load-bearing invariants — thresholds-as-snapshot, reasons-as-bare-codes, one-event-per-detected-token — that the rest of the system depends on.
