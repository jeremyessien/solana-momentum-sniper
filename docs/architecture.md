...
The enrichment layer takes a detected token and gathers all the additional information we need to evaluate it. This includes holder distribution, liquidity pool details, creator wallet history, transaction patterns from the early minutes of trading, contract security flags, and any third-party risk signals. The output is an enriched candidate that strategies can evaluate.

The reason this is its own module is that the axis of change is "what data we want to gather and from where." As we learn what data is most predictive, we will add new enrichment sources or remove ones that prove unhelpful. Caching, parallel fetching, and partial-result handling all live in this module rather than being scattered throughout the code.

Enrichment is the most expensive part of the pipeline because it involves many external requests. The module is built to fetch all data sources in parallel using asynchronous concurrency primitives, similar to using `Future.wait` in Dart. This means total enrichment time is bounded by the slowest source rather than the sum of all sources. The module is also explicitly designed to handle partial failures. If one data source fails, the candidate continues with degraded data rather than being rejected. The enriched candidate type carries metadata about which sources succeeded and which did not, allowing strategies to decide for themselves whether they require complete information.
...
A `TokenWithFullContext` represents a detected token after enrichment. It embeds the original detected token and adds all the additional fields gathered during enrichment. Because enrichment is fallible, each enriched field carries a confidence indicator showing whether the data is complete, partial, or missing. Strategies can pattern match on this confidence to decide whether they require complete information.
...
A `RejectedEntryReport` and a `FailedAnalysisReport` capture the negative outcomes that we still want to record for analysis. Even when something goes wrong, we capture the fact and the reason, because patterns in failures are informative.
...
The infrastructure layer's Helius adapter is maintaining a gRPC stream connection to the Solana network, listening for transactions involving the Pump.fun program. A new transaction arrives. The adapter parses it and recognizes it as a token creation. It publishes the raw event to the event bus.

The detection layer is subscribed to the relevant infrastructure events. It receives the raw event, extracts the relevant fields, normalizes them into our internal `DetectedToken` shape, and publishes a `NewTokenLaunchDetected` event with the detected token as the payload.

Several modules are subscribed to this event. The data store records the detection for our analytical archive. Any debug logger we have running captures the detection. Most importantly, the enrichment layer receives the event and begins its work.

Enrichment fires off several parallel queries. It asks Helius for the holder distribution of the new token. It asks DexScreener for the current liquidity pool depth. It asks RugCheck for any risk signals. It asks Helius for the creator wallet's history of previous token launches. It analyzes the early transaction patterns to identify wallet diversity and buy-sell ratio. All of these happen concurrently, similar to how `Future.wait` parallelizes operations in Dart.

When all parallel queries complete or time out, the enrichment layer assembles the results into a `TokenWithFullContext` and publishes a `TokenAnalysisCompleted` event. If some queries failed, the affected fields are marked with partial or missing confidence rather than the whole candidate being rejected.
