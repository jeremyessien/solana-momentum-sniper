...
The infrastructure layer's Helius adapter is maintaining a WebSocket subscription to the Solana network, listening for log events from the Pump.fun program. A new transaction arrives. The adapter receives the program's log notification, fetches the full transaction details for parsing, and recognises it as a token creation. It publishes the raw event to the event bus.
