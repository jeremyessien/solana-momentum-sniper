...
For Solana RPC and WebSocket access, **`@solana/kit`** is the primary client, configured with the Helius endpoint URL for both HTTP RPC and WebSocket subscriptions. The free Helius tier supports standard JSON-RPC WebSocket subscriptions including `logsSubscribe` and `programSubscribe`, which is sufficient for Phase 1 token detection — we subscribe to Pump.fun program logs and fetch full transaction details on demand via `getTransaction`. The **Helius SDK** is layered on top only when Helius-specific Enhanced features become operationally needed, including their Enhanced WebSocket `transactionSubscribe` (which delivers full filtered transactions in one call rather than the two-hop logs-then-fetch pattern) and their priority fee estimation. Those Enhanced features require a paid Helius plan ($24.50–$499/mo as of May 2026). gRPC LaserStream remains available for projects that need its lower latency, but it is not justified at our scale — the architecture document's selectivity-not-speed framing explicitly trades raw latency for discipline. We upgrade tiers when rate limits or richer-data needs make it operationally necessary, not preemptively.
